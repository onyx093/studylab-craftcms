...
const FeatureSection = ({
  ...
  return (
    <FeatureSectionWrapper id="featureSection">
      <Container>
        <Box {...sectionHeader}>
          <Text content="Feedback" {...sectionSubTitle} />
          <Heading
            content="Feedback From Educators"
            {...sectionTitle}
          />
        </Box>
        <Box className="row" {...row}>
          {boards.map((feature, index) => (
            <Box className="col" {...col} key={`feature-${index}`}>
              <FeatureBlock
                icon={<img src={feature.boardLogo[0].url} />}
                wrapperStyle={blockWrapperStyle}
                iconStyle={iconStyle}
                contentStyle={contentStyle}
                title={<Heading content={feature.boardName} {...featureTitle} />}
              />
            </Box>
          ))}
        </Box>
